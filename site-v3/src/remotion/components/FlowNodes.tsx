  ...
  return (
    <div style={{ position: "relative", width: "100%", height: 260 }}>
      <svg viewBox="0 0 1300 260" style={{ position: "absolute", inset: 0, overflow: "visible" }}>
        ...
        <path d="M110 130 H1190" stroke={colors.primary} strokeWidth="10" strokeLinecap="round" strokeDasharray="1080" strokeDashoffset={1080 - interpolate(progress, [0, steps.length - 1], [0, 1080])} filter="drop-shadow(0 0 14px rgba(255,139,61,.7))" />
      </svg>
        ...
        {steps.map((step, index) => {
          ...
          return (
            <div key={step} style={{ display: "grid", placeItems: "center", gap: 18, transform: `scale(${interpolate(scale, [0, 1], [.78, 1])})`, opacity: interpolate(scale, [0, 1], [0, 1]) }}>
              <div style={{ width: 98, height: 98, borderRadius: 99, display: "grid", placeItems: "center", border: `4px solid ${active ? colors.primary : colors.hairline}`, background: active ? "rgba(255,139,61,.16)" : colors.surfaceCard, boxShadow: active ? "0 0 34px rgba(255,139,61,.38)" : "none", color: active ? colors.primary : colors.body, fontSize: 30, fontWeight: 900 }}>{index + 1}</div>
