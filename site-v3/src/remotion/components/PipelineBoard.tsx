  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 22, height: "100%" }}>
      {columns.map((column, columnIndex) => (
        <div key={column.title} style={{ border: "1px solid rgba(255,255,255,.11)", borderRadius: 20, background: "rgba(14,34,51,.86)", padding: 22 }}>
          ...
          <div style={{ display: "grid", gap: 16 }}>
            {column.cards.map((card, cardIndex) => {
              ...
              return (
                <div key={card} style={{ minHeight: 88, border: "1px solid rgba(255,139,61,.24)", borderRadius: 16, background: colors.surfaceCard, padding: 18, opacity, transform: `translateY(${interpolate(opacity, [0, 1], [18, 0])}px)` }}>
