      ...
      {messages.map((message, index) => {
        ...
        return (
          <div key={`${message.label}-${index}`} style={{ display: "flex", justifyContent: right ? "flex-end" : "flex-start", opacity: appear, transform: `translateY(${interpolate(appear, [0, 1], [22, 0])}px)` }}>
            <div style={{ width: 445, border: `1px solid ${right ? "rgba(255,139,61,.45)" : "rgba(255,255,255,.13)"}`, borderRadius: 20, background: right ? "rgba(255,139,61,.13)" : colors.surfaceCard, padding: 19, boxShadow: right ? "0 0 32px rgba(255,139,61,.12)" : "0 16px 54px rgba(0,0,0,.28)" }}>
