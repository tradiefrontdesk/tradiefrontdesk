    ...
    <VideoShell label="The reframe" cta="Fix capture before adding volume">
      <div style={{ position: "relative", height: "100%" }}>
        ...
        <svg viewBox="0 0 964 690" style={{ position: "absolute", left: 0, right: 0, top: 118, width: "100%", height: 690, overflow: "visible" }}>
          ...
          <rect x="34" y="34" width="896" height="610" rx="0" fill="rgba(14,34,51,.9)" stroke={colors.primary} strokeWidth="4" />
          ...
          <g opacity={leakFade}>
            <rect x="338" y="216" width="286" height="120" rx="0" fill="rgba(14,34,51,.92)" stroke={colors.error} strokeWidth="4" />
            ...
            {leaks.map((leak, index) => {
              ...
              return (
                <g key={leak} opacity={interpolate(frame - index * 18, [24, 50], [0, 1], clamp)}>
                  <rect x={x} y={y} width="330" height="36" rx="0" fill="rgba(14,34,51,.96)" stroke={colors.error} strokeWidth="2" />
          ...
          <g transform={`translate(262 218) scale(${interpolate(systemScale, [0, 1], [0.72, 1], clamp)})`} opacity={interpolate(systemScale, [0, 1], [0, 1], clamp)}>
            <rect x="0" y="0" width="440" height="286" rx="0" fill="rgba(14,34,51,.98)" stroke={colors.primary} strokeWidth="4" />
            <rect x="24" y="28" width="392" height="52" rx="0" fill="rgba(255,90,31,.14)" stroke={colors.primary} strokeWidth="2" />
            ...
            {steps.map((step, index) => {
              ...
              return (
                <g key={step} transform={`translate(${x} 132)`}>
                  <circle r={active ? 28 * (index === Math.floor(flowProgress * steps.length) ? pulse : 1) : 24} fill={active ? colors.primary : colors.surfaceCard} stroke={active ? "#0E2233" : "rgba(255,255,255,.28)"} strokeWidth="4" />
            ...
            <rect x="34" y="220" width="372" height="38" rx="0" fill="rgba(255,90,31,.12)" stroke={colors.primary} strokeWidth="2" />
          ...
          <g opacity={interpolate(frame, [246, 306], [0, 1], clamp)}>
            <rect x="104" y="560" width="756" height="50" rx="0" fill="rgba(14,34,51,.96)" stroke={colors.primary} strokeWidth="3" />
