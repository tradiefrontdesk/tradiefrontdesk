...
const SmallCard = ({ children, active = true, style }: { children: ReactNode; active?: boolean; style?: React.CSSProperties }) => (
  <SquareCard
    style={{
      padding: 15,
      borderColor: active ? "rgba(255,139,61,.42)" : "rgba(255,255,255,.14)",
      boxShadow: active ? "0 18px 56px rgba(0,0,0,.34), 0 0 30px rgba(255,139,61,.16)" : "0 18px 44px rgba(0,0,0,.28)",
      ...style,
    }}
  >
    {children}
  ...
  return (
    <SquareCard style={{ padding: 20 }}>
      <svg viewBox="0 0 760 430" style={{ display: "block", width: "100%", height: 430 }}>
        <circle cx="380" cy="216" r={84 + Math.sin(frame / 14) * 5} fill="rgba(255,139,61,.09)" stroke="rgba(255,139,61,.42)" strokeWidth="3" />
        ...
        {positions.map(([x, y], index) => {
          ...
          return (
            <g key={`${x}-${y}`} opacity={reveal}>
              <line x1="380" y1="216" x2={x + 80} y2={y + 46} stroke={active ? "rgba(255,139,61,.6)" : "rgba(255,255,255,.13)"} strokeWidth="4" />
              ...
              <rect x={x + 18} y={y + 58} width={95 * pop} height="7" rx="4" fill="rgba(255,139,61,.52)" />
            </g>
          );
        })}
      </svg>
...
const ComparisonVisual = ({ story }: { story: StoryVideoConfig }) => {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 80px 1fr", gap: 14, alignItems: "stretch" }}>
      <SquareCard style={{ padding: 18, minHeight: 380, borderColor: "rgba(255,77,77,.35)" }}>
        <div style={{ color: colors.error, fontSize: 18, fontWeight: 950, textTransform: "uppercase" }}>Old way</div>
        <div style={{ display: "grid", gap: 14, marginTop: 20 }}>
          {oldItems.map((item, index) => (
            <div key={item} style={{ opacity: clamp(frame - index * 18, [0, 20], [0, 1]), padding: 14, borderRadius: 12, background: "rgba(255,77,77,.08)", color: colors.body, fontSize: 21, fontWeight: 850 }}>
              {item}
            </div>
          ))}
        </div>
      ...
      <SquareCard style={{ padding: 18, minHeight: 380 }}>
        ...
        <div style={{ display: "grid", gap: 14, marginTop: 20 }}>
          {systemItems.map((item, index) => (
            <div key={item} style={{ opacity: clamp(frame - 80 - index * 18, [0, 20], [0, 1]), padding: 14, borderRadius: 12, background: "rgba(255,139,61,.09)", color: colors.text, fontSize: 21, fontWeight: 900 }}>
              {item}
            </div>
          ))}
        </div>
        ...
        {story.cards.slice(0, 4).map((card, index) => {
          ...
          return (
            <g key={card} opacity={clamp(frame - 75 - index * 18, [0, 22], [0, 1])}>
              <rect x={x} y={y} width="270" height="66" rx="15" fill={colors.surfaceCard} stroke={index === 0 ? colors.primary : "rgba(255,139,61,.24)"} strokeWidth="2" />
  ...
  return (
    <SquareCard style={{ padding: 18 }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10 }}>
        ...
        {story.cards.slice(0, 4).map((card, index) => (
          <div key={card} style={{ opacity: clamp(frame - index * 22, [0, 24], [0, 1]), minHeight: 215, padding: 14, borderRadius: 14, border: "1px solid rgba(255,255,255,.13)", background: index === 2 ? "rgba(255,139,61,.1)" : "rgba(255,255,255,.045)" }}>
