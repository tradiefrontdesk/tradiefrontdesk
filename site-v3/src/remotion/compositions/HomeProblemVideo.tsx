    ...
    <VideoShell label="The problem" cta="Busy process, leaking opportunities">
      <div style={{ position: "relative", height: "100%" }}>
        ...
        <svg viewBox="0 0 964 694" style={{ position: "absolute", inset: "108px 0 0", width: "100%", height: 694, overflow: "visible" }}>
          ...
          <rect x="32" y="30" width="900" height="612" rx="28" fill="rgba(14,34,51,.76)" stroke="rgba(255,139,61,.22)" />
          ...
          {breakpoints.map((point, index) => {
            ...
            return (
              <g key={point.label} opacity={appear}>
                <circle cx={point.x} cy={point.y} r={22 + crack * 4} fill="rgba(14,34,51,.94)" stroke={isCracked ? colors.error : colors.primary} strokeWidth="5" filter="url(#problemGlow)" />
                ...
                <rect x={point.x - 96} y={point.y + 42} width="192" height="74" rx="15" fill="rgba(14,34,51,.96)" stroke={isCracked ? "rgba(255,77,77,.48)" : "rgba(255,139,61,.28)"} />
          ...
          <g opacity={interpolate(frame, [184, 246], [0, 1], clamp)}>
            <rect x="104" y="500" width="756" height="86" rx="22" fill="rgba(255,77,77,.12)" stroke="rgba(255,77,77,.42)" />
          ...
          <g opacity={interpolate(frame, [270, 332], [0, 1], clamp)}>
            <rect x="132" y="606" width="700" height="46" rx="15" fill="rgba(255,139,61,.12)" stroke="rgba(255,139,61,.36)" />
