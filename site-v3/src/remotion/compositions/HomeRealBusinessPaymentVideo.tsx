...
const Check = ({ x, y, show }: { x: number; y: number; show: number }) => (
  <g opacity={show}>
    <circle cx={x} cy={y} r="18" fill={colors.primary} filter="drop-shadow(0 0 16px rgba(255,139,61,.65))" />
        ...
        <svg viewBox="0 0 964 560" style={{ position: "absolute", inset: "118px 0 0", width: "100%", height: 520, overflow: "visible" }}>
          <defs>
            <linearGradient id="payGlow" x1="0" x2="1">
              <stop offset="0" stopColor="rgba(255,139,61,.16)" />
              <stop offset="1" stopColor="rgba(255,139,61,.02)" />
            </linearGradient>
          </defs>
          ...
          <g transform={`translate(${(1 - checkout) * -42} ${(1 - checkout) * 24}) scale(${0.92 + checkout * 0.08})`} opacity={checkout}>
            <rect x="46" y="26" width="380" height="456" rx="26" fill={colors.surfaceCard} stroke="rgba(255,139,61,.42)" strokeWidth="3" filter="drop-shadow(0 24px 48px rgba(0,0,0,.38))" />
          ...
          <path d="M426 254 C500 254 492 248 560 248" stroke={colors.primary} strokeWidth="9" strokeLinecap="round" fill="none" strokeDasharray="142" strokeDashoffset={142 - systemLine * 142} filter="drop-shadow(0 0 15px rgba(255,139,61,.72))" />
          ...
          {nodes.map(([label, x, y], index) => {
            ...
            return (
              <g key={label} opacity={reveal} transform={`translate(${x} ${y}) scale(${active ? pulse : 1})`}>
                <circle r="54" fill={active ? "rgba(255,139,61,.14)" : colors.surfaceCard} stroke={active ? colors.primary : "rgba(255,255,255,.16)"} strokeWidth="3" />
          ...
          <g opacity={clamp(frame - 278, [0, 24], [0, 1])}>
            <rect x="536" y="410" width="374" height="76" rx="18" fill={colors.surfaceCard} stroke="rgba(255,139,61,.38)" />
          ...
          <rect x={sweep * 8 - 160} y="0" width="120" height="520" fill="rgba(255,139,61,.08)" transform="skewX(-18)" opacity="0.8" />
        </svg>
      </div>
    </VideoShell>
  );
