...
- Primary CTA: **Book a Free Front Desk Audit** (`/free-audit`) — the full phrase is the default CTA band button (`CTASection.astro`); the header and sticky mobile bar carry the short label **Book Free Audit** pointing at the same route.
- Secondary fallback: call the business directly — the header shows the NZ number (the first entry in `contactDetails.phones`); all three regional numbers (NZ/AU/US) appear in the footer — for a visitor not ready to fill in a form; browsing the packages (`/pricing`) or the worked examples (`/case-studies`) are the next-best soft paths for a visitor still evaluating.
