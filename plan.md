...
| §7 "HowTo schema for guides" | **Implement, but expect nothing from Google.** Google deleted the HowTo rich result documentation on 14 September 2023; it renders on neither desktop nor mobile. Built anyway — valid schema.org, honest description of a procedural page, and read by Bing and AI answer engines. See `src/layouts/BaseLayout.astro`. |
| §7 / §2 FAQPage schema | **Keep, but the same caveat now applies.** Google stopped showing FAQ rich results on 7 May 2026 and removed the documentation on 15 June 2026. The markup stays: unused structured data is harmless and non-Google consumers still parse it. Nobody should re-litigate stripping it out. |
